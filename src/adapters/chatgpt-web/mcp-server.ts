...
const ZERO_RISK_MCP_INSTRUCTIONS = [
  "For each pasted Fame Codex request, begin with codex_turn_start using the request_id in its request block.",
...
export async function runChatGptMcpServer(options: {
  ...
  if (contract === "safe") {
    server.registerTool(
      "codex_turn_start",
      {
        title: "Connect a Fame Codex Zero Risk request",
        description: "Connect the request_id included in the pasted Fame Codex request so its Codex tools can be used.",
        inputSchema: {
          request_id: turnTokenSchema,
        },
        outputSchema: {
