...
- Run its browser smoke test and initialize the DEV profile. Complete MCP setup only when testing
  ...
  tunnel. Create the ChatGPT connector as `Fame Codex Native2 DEV`; keep `Fame Codex Native2` unchanged.
...
the launcher-owned DEV tunnel ready so ChatGPT can create and validate `Fame Codex Native2 DEV` before a
...
Set `FAME_CODEX_DEV_HOME` to choose another absolute DEV home. Generic `--home`,
`FAME_CODEX_HOME`, `CODEX_HOME`, and `FAME_CODEX_LAUNCHER_DATA_DIR` never collapse the DEV
