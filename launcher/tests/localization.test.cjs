...
for (const language of Object.keys(languages).filter(language => language !== "en")) test(`${language} runtime localization preserves literal connector names and endpoints`, () => {
  ...
  assert.equal(
    localizeRuntimeMessage(copy, "Responses proxy is healthy on 127.0.0.1:17842", "proxy", language),
    copy.doctorProxyHealthy.replace("{endpoint}", () => "127.0.0.1:17842"),
  );
});

test("runtime message localization preserves other languages and unknown backend messages", () => {
  const { copyFor, localizeRuntimeMessage } = loadI18nModule();
  const connectorNames = ["Fame Codex Native2", "Native $&", "Native $'", "Native $`", 'Native "quoted"', "Native \\path"];
  ...
  assert.equal(
    localizeRuntimeMessage(copyFor("ja"), 'ChatGPT connector "Fame Codex Native2" is available', "wrong-id", "ja"),
    'ChatGPT connector "Fame Codex Native2" is available',
  );
  assert.equal(
  ...
  assert.equal(
    localizeRuntimeMessage(copyFor("ja"), 'ChatGPT connector "Fame Codex Native2" is available (warning)', "connector", "ja"),
    'ChatGPT connector "Fame Codex Native2" is available (warning)',
  );
});
