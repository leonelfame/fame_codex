...
const REPOSITORY = "leonelfame/fame_codex";
...
function buildJob({ version, platform, executablePath, assetPath, stagingRoot, tempRoot, logPath }) {
  ...
  if (platform === "linux") {
    const target = process.env.FAME_CODEX_APPIMAGE?.trim()
      || process.env.CODEX_WEB_GPT_APPIMAGE?.trim()
    ...
    const wrapper = process.env.FAME_CODEX_LAUNCHER_EXECUTABLE?.trim()
      || process.env.CODEX_WEB_GPT_LAUNCHER_EXECUTABLE?.trim();
