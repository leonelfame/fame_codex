...
test("browser configuration rejects retired connector identities before opening a turn", () => {
  for (const appName of ["Codex Native", "Codex Native2"]) {
    expect(() => resolveBrowserConfig({
      adapter: "chatgpt-web",
...
test("connector selection re-resolves the active composer after ChatGPT replaces it", async () => {
    ...
    locator: (selector: string) => {
      ...
      return {
        filter: (options: { hasText: string; visible: boolean }) => {
          expect(options).toEqual({ hasText: "Fame Codex Native2", visible: true });
  ...
  const page = {
    ...
    getByText: (text: string, options: { exact: boolean }) => {
      expect(text).toBe("Fame Codex Native2");
  ...
  const resolved = await selectConnector.call({
    config: { appName: "Fame Codex Native2" },
...
test("connector selection moves highlight to the exact hidden-viewport row before Enter", async () => {
  ...
  await expect(selectConnector.call({
    config: { appName: "Fame Codex Native2 DEV" },
...
test("connector verification preserves the host-refreshed catalog evidence", async () => {
  ...
  const visibleRows = {
    allInnerTexts: async () => catalogFresh ? ["Fame Codex Native2"] : ["Another connector"],
  };
  const menuRows = {
  ...
  let prepared = 0;
  const fixture = {
    config: { appName: "Fame Codex Native2", browserDiagnosticsPath: diagnosticsRoot },
  ...
  try {
    await expect(prototype.verifyConnectorExclusive.call(fixture)).rejects.toThrow(
      'connector menu opened but exposed no row named "Fame Codex Native2"',
    );
...
test("successful connector verification clears the proven selection before releasing the page", async () => {
  ...
  try {
    const result = await verifyConnectorExclusive.call({
      config: { appName: "Fame Codex Native2 DEV", browserDiagnosticsPath: diagnosticsRoot },
      ensurePage: async () => page,
      prepareTemporaryChatSurface: async (_page: unknown, capture: (checkpoint: string) => Promise<void>) => {
        calls.push("prepare");
        await capture("composer-ready");
    ...
    expect(result).toBe("Fame Codex Native2 DEV");
...
test("Think slash toggles only when needed, preserves connectors, and normal Luna clears it", async () => {
  ...
  state.connectors = ["Fame Codex Native2"];
  const checkpoints: string[] = [];
  ...
  expect(state.connectors).toEqual(["Fame Codex Native2"]);
...
test("Think attachment runs after fresh connector selection and rechecks retained and Browser-only turns", async () => {
  ...
  for (const [localTools, retained] of [[true, false], [true, true], [false, false]]) {
    ...
    const worker = {
      ...
      selectConnector: async () => { connectorSelections += 1; ui.state.connectors = ["Fame Codex Native2"]; return ui.composer; },
    ...
    if (localTools && !retained) expect(ui.state.connectors).toEqual(["Fame Codex Native2"]);
    if (retained) {
...
test("Think attachment rolls back a lost connector and never inserts the prompt", async () => {
  ...
  let cleanup = 0;
  const worker = {
    selectConnector: async () => { ui.state.connectors = ["Fame Codex Native2"]; return ui.composer; },
