...
async function loginCommand(args: string[]): Promise<void> {
  ...
  if (!launcherControl) {
    ...
    if (config.browserHost === "launcher") {
      throw new Error("ChatGPT login is owned by the launcher; open Fame Codex and use its Sign in step");
    }
...
async function uninstallCommand(args: string[]): Promise<void> {
  ...
  if (config?.browserHost === "launcher" && !launcherControl) {
    throw new Error(
      "Launcher-owned integration must be removed from Fame Codex Settings so the active runtime can be drained safely.",
    );
  }
...
async function main(): Promise<void> {
  ...
  if (home) process.env.FAME_CODEX_HOME = home;
  ...
  if (command === "dev" && home) {
    throw new Error("--home does not apply to DEV mode; use FAME_CODEX_DEV_HOME for an explicit isolated DEV profile");
  }
