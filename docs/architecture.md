...
broker attached to that tunnel for the command's lifetime. The distinct `Fame Codex Native2 DEV`
connector reaches the same MCP server and turn-token contract without requiring any Responses
daemon or colliding with the production `Fame Codex Native2` connector.
...
under `~/.fame-codex-dev` by default.

The ChatGPT connector name is also the public MCP ABI identity. The direct turn-token contract uses
`Fame Codex Native2`; the retired `Codex Native` and `Codex Native2` identities are never selected or refreshed in place. Setup
...
Repository DEV mode uses `Fame Codex Native2 DEV` so the same ChatGPT account can keep both production
