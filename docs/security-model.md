...
The direct turn-token MCP schema is attached only through the `Fame Codex Native2` connector identity.
The older `Codex Native` and `Codex Native2` connectors are treated as legacy and are never selected
as fallbacks. This prevents a cached legacy schema from being mistaken for the current capability
contract.
