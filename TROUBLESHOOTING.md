...
1. Install the [latest release](https://github.com/leonelfame/fame_codex/releases/latest). Quit
   **Fame Codex** before running the installer again; updating preserves its private ChatGPT
...
OmniRoute, Codex++, CC Switch, or a manually configured provider may replace the Fame Codex route
for the whole installation or only for the process they launch.

Choose one route owner:

- To use Fame Codex, disable the other wrapper's provider/proxy mode, run **Repair Codex setup**,
...
- To switch away cleanly, use **Settings → Remove Codex integration** first. This restores the exact
  route that existed before Fame Codex was installed.
...
tracked in [#205](https://github.com/leonelfame/fame_codex/issues/205), but is not supported today.
...
- Passkey-only macOS accounts have a known open issue: [#209](https://github.com/leonelfame/fame_codex/issues/209).
...
- a newly created connector named exactly **Fame Codex Native2**;
...
Do not rename or refresh an old **Codex Native** or **Codex Native2** connector. ChatGPT caches the
public MCP contract by connector identity, so create **Fame Codex Native2** as a new connector.
...
To update, quit **Fame Codex** and run the same installer command from the README. The installer
...
4. If Full harness was configured and is no longer wanted, separately delete **Fame Codex Native2**, its
