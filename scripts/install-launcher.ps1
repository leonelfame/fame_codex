...
$Repository = if ($env:FAME_CODEX_REPOSITORY) { $env:FAME_CODEX_REPOSITORY } elseif ($env:CODEX_WEB_GPT_REPOSITORY) { $env:CODEX_WEB_GPT_REPOSITORY } else { "leonelfame/fame_codex" }
if ($Repository -notmatch '^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$') {
  throw "Invalid GitHub repository: $Repository"
}
$Version = if ($env:FAME_CODEX_VERSION) { $env:FAME_CODEX_VERSION } else { $env:CODEX_WEB_GPT_VERSION }
if (-not $Version) {
