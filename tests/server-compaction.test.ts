...
test("compacts ChatGPT Web v1 through a dedicated read-only browser summarization turn", async () => {
  ...
  const response = await compactRequest(new Request("http://127.0.0.1:17842/v1/responses/compact", {
    method: "POST",
...
test("returns exactly one native compaction item for a ChatGPT Web v2 request", async () => {
  const providers: CodexProviderConfig[] = [];
  const config = defaultConfig("full");
  const response = await responseRequest(new Request("http://127.0.0.1:17842/v1/responses", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
      model,
...
test("v2 recompaction reads the previous checkpoint once and replaces it with one new compaction item", async () => {
  const config = defaultConfig("full");
  const firstResponse = await responseRequest(new Request("http://127.0.0.1:17842/v1/responses", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
      model,
  ...
  const secondResponse = await responseRequest(new Request("http://127.0.0.1:17842/v1/responses", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
      model,
