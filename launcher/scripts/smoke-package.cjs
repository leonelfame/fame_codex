...
const scratch = fs.mkdtempSync(path.join(os.tmpdir(), "fame-codex-package-smoke-"));
...
function smokeEnvironment() {
  return {
    ...process.env,
    TMPDIR: scratch,
    FAME_CODEX_LAUNCHER_DATA_DIR: path.join(scratch, "launcher-data"),
...
try {
  ...
  // A first packaged launch installs and verifies the full runtime tree. Windows
  // antivirus can make that one-time copy substantially slower than a normal launch.
