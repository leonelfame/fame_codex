...
const en = {
  ...
  devConnectorIsolationNotice: "Create a separate connector with the exact DEV name below. Keep Fame Codex Native2 unchanged so production and repository development can run side by side.",
  ...
  supportBody: "Star the repository and open my X profile to help more developers find Fame Codex. I share product updates and useful Codex tips on X.",
  star: "Star on GitHub",
  ...
  manualMcpStepThreeBody: "Create the separate connector shown below with the exact name Fame Codex Zero Risk and select it yourself in every Zero Risk turn. Zero Risk requires its own tunnel and credentials, separate from Automatic mode. The launcher never inspects the ChatGPT DOM in this mode.",
  connectorMigrationNotice: "Upgrading from Codex Native / Codex Native2? Leave the old connector untouched and create Fame Codex Native2 as a new connector. Do not rename or refresh Codex Native / Codex Native2. Verify runtime rejects a legacy-only setup.",
  manualConnectorNotice: "Fame Codex Zero Risk is a separate connector. Select it yourself before sending every Zero Risk prompt; the launcher intentionally does not inspect ChatGPT to verify that choice.",
  ...
  manualPromptInstruction: "The prompt is already in your clipboard. Paste it into the open ChatGPT tab, choose the model and effort you want, select the Fame Codex Zero Risk connector, send it, then confirm below.",
  manualPromptCopy: "Copy prompt",
  manualPromptCancel: "Cancel turn",
  manualPromptSent: "Sent",
  manualPromptWaiting: "Waiting for Fame Codex Zero Risk to connect",
...
const zh: Record<keyof typeof en, string> = {
  ...
  devConnectorIsolationNotice: "请使用下方准确的 DEV 名称新建独立连接器。保留 Fame Codex Native2 不变，让生产使用和仓库开发可以同时运行。",
  ...
  continue: "继续",
  supportTitle: "开始之前",
  supportBody: "请为仓库 Star，并打开我的 X 主页，帮助更多开发者发现 Fame Codex。我会在 X 分享产品更新和实用的 Codex 技巧。",
  ...
  manualMcpStepThreeBody: "请使用下方显示的准确名称 Fame Codex Zero Risk 创建独立连接器，并在每次引导式手动回合中自行选择它。Zero Risk 必须使用与自动模式分开的 Tunnel 和凭据。此模式不会执行 DOM 检查。",
  connectorMigrationNotice: "从 Codex Native / Codex Native2 升级？请保留旧连接器不变，并新建名为 Fame Codex Native2 的连接器。不要重命名或刷新 Codex Native / Codex Native2。仅有旧连接器时，运行时验证会明确失败。",
  manualConnectorNotice: "Fame Codex Zero Risk 是独立连接器。每次发送引导式提示前都需要自行选择它；启动器有意不检查 ChatGPT DOM 来验证该选择。",
  ...
  manualPromptInstruction: "提示已复制到剪贴板。请将其粘贴到打开的 ChatGPT 标签页，选择所需模型和 effort，再选择 Fame Codex Zero Risk 连接器，发送后在下方确认。",
  manualPromptCopy: "复制提示",
  manualPromptCancel: "取消任务",
  manualPromptSent: "已发送",
  manualPromptWaiting: "等待 Fame Codex Zero Risk 连接",
...
const ja: Record<keyof typeof en, string> = {
  ...
  devConnectorIsolationNotice: "下記の正確な DEV 名で個別のコネクタを作成してください。Fame Codex Native2 は変更せず、本番利用とリポジトリ開発を並行して実行できる状態を保ちます。",
  ...
  continue: "続ける",
  supportTitle: "開始する前に",
  supportBody: "より多くの開発者に Fame Codex を知ってもらうため、リポジトリに Star を付け、私の X プロフィールを開いてください。X では製品のアップデートや便利な Codex の使い方を紹介しています。",
  star: "GitHub で Star",
  ...
  manualMcpStepThreeBody: "下に表示される正確な名前 Fame Codex Zero Risk で専用コネクタを作成し、ガイド付き手動ターンごとに自分で選択してください。Zero Risk では、自動モードとは別の Tunnel と認証情報が必要です。このモードでは DOM 検査を行いません。",
  connectorMigrationNotice: "Codex Native / Codex Native2 からアップグレードする場合は、古いコネクタを変更せず、新しい Fame Codex Native2 コネクタを作成してください。Codex Native / Codex Native2 の名前変更や更新は行わないでください。古いコネクタしかない場合、ランタイム検証は明示的に失敗します。",
  manualConnectorNotice: "Fame Codex Zero Risk は専用コネクタです。ガイド付きプロンプトを送信する前に毎回自分で選択してください。ランチャーはその選択を確認するための ChatGPT DOM 検査を意図的に行いません。",
  ...
  manualPromptInstruction: "プロンプトはクリップボードにコピー済みです。開いている ChatGPT タブに貼り付け、モデルと effort、Fame Codex Zero Risk コネクタを選んで送信し、下で確認してください。",
  manualPromptCopy: "プロンプトをコピー",
  manualPromptCancel: "ターンをキャンセル",
  manualPromptSent: "送信済み",
  manualPromptWaiting: "Fame Codex Zero Risk の接続を待機中",
...
const ko: Record<keyof typeof en, string> = {
  ...
  devConnectorIsolationNotice: "아래의 정확한 DEV 이름으로 별도의 커넥터를 만드세요. Fame Codex Native2는 변경하지 않아 프로덕션 환경과 저장소 개발 환경을 동시에 사용할 수 있도록 유지하세요.",
  ...
  supportBody: "더 많은 개발자가 Fame Codex를 찾을 수 있도록 저장소에 Star를 누르고 제 X 프로필을 열어주세요. X에서 제품 업데이트와 유용한 Codex 활용 팁을 공유합니다.",
  star: "GitHub에서 Star",
  starred: "GitHub 열림",
  follow: "X 열기",
  followed: "X 열림",
  ...
  manualMcpStepThreeBody: "아래에 표시된 정확한 이름인 Fame Codex Zero Risk로 별도의 커넥터를 만들고, 모든 Zero Risk 턴에서 직접 선택하세요. Zero Risk는 자동 모드와 별도의 터널 및 인증 정보가 필요합니다. 이 모드에서는 런처가 ChatGPT DOM을 검사하지 않습니다.",
  connectorMigrationNotice: "Codex Native / Codex Native2에서 업그레이드하는 경우 기존 커넥터는 그대로 두고 Fame Codex Native2라는 새 커넥터를 만드세요. Codex Native / Codex Native2의 이름을 변경하거나 새로고침하지 마세요. 기존 커넥터만 있는 구성은 런타임 검증에서 거부됩니다.",
  manualConnectorNotice: "Fame Codex Zero Risk는 별도의 커넥터입니다. 각 Zero Risk 프롬프트를 보내기 전에 직접 선택하세요. 런처는 해당 선택을 확인하기 위해 ChatGPT DOM을 의도적으로 검사하지 않습니다.",
  ...
  manualPromptInstruction: "프롬프트가 이미 클립보드에 복사되어 있습니다. 열려 있는 ChatGPT 탭에 붙여넣고 원하는 모델과 effort를 선택한 다음 Fame Codex Zero Risk 커넥터를 선택하여 전송하고 아래에서 확인하세요.",
  manualPromptCopy: "프롬프트 복사",
  manualPromptCancel: "턴 취소",
  manualPromptSent: "전송됨",
  manualPromptWaiting: "Fame Codex Zero Risk 연결 대기 중",
...
const zhTW: Record<keyof typeof en, string> = {
  ...
  devConnectorIsolationNotice: "請使用下方正確的 DEV 名稱新增獨立連接器。保留 Fame Codex Native2 不變，讓正式環境與儲存庫開發可以同時執行。",
  ...
  continue: "繼續",
  supportTitle: "開始之前",
  supportBody: "請為儲存庫按 Star，並開啟我的 X 主頁，幫助更多開發者發現 Fame Codex。我會在 X 分享產品更新和實用的 Codex 技巧。",
  ...
  manualMcpStepThreeBody: "請使用下方顯示的正確名稱 Fame Codex Zero Risk 建立獨立連接器，並在每次引導式手動回合中自行選擇它。Zero Risk 必須使用與自動模式分開的 Tunnel 與憑證。此模式不會執行 DOM 檢查。",
  connectorMigrationNotice: "從 Codex Native / Codex Native2 升級？請保留舊連接器不變，並新增名為 Fame Codex Native2 的連接器。不要重新命名或重新整理 Codex Native / Codex Native2。僅有舊連接器時，執行階段驗證會明確失敗。",
  manualConnectorNotice: "Fame Codex Zero Risk 是獨立連接器。每次傳送引導式提示前都需要自行選擇它；啟動器有意不檢查 ChatGPT DOM 來驗證該選擇。",
  ...
  manualPromptInstruction: "提示已複製到剪貼簿。請將其貼到開啟的 ChatGPT 分頁，選擇所需模型與 effort，再選擇 Fame Codex Zero Risk 連接器，傳送後在下方確認。",
  manualPromptCopy: "複製提示",
  manualPromptCancel: "取消任務",
  manualPromptSent: "已傳送",
  manualPromptWaiting: "等待 Fame Codex Zero Risk 連線",
