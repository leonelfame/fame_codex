...
REPOSITORY="${FAME_CODEX_REPOSITORY:-${CODEX_WEB_GPT_REPOSITORY:-leonelfame/fame_codex}}"
VERSION="${FAME_CODEX_VERSION:-${CODEX_WEB_GPT_VERSION:-}}"
OS="$(uname -s)"
MACHINE="$(uname -m)"
...
if [ "$OS" = "Darwin" ]; then
  INSTALL_DIR="${FAME_CODEX_APPLICATIONS_DIR:-${CODEX_WEB_GPT_APPLICATIONS_DIR:-/Applications}}"
...
LIB_DIR="${FAME_CODEX_LIB_DIR:-${CODEX_WEB_GPT_LIB_DIR:-$HOME/.local/lib/fame-codex}}"
BIN_DIR="${FAME_CODEX_BIN_DIR:-${CODEX_WEB_GPT_BIN_DIR:-$HOME/.local/bin}}"
TARGET_DIR="$LIB_DIR/$VERSION"
TARGET="$TARGET_DIR/Fame Codex.AppImage"
WRAPPER="$BIN_DIR/fame-codex"
CORE_HOME="${FAME_CODEX_HOME:-$HOME/.fame-codex}"
...
RUNNER_QUOTED="$(shell_quote "$RUNNER")"
{
  printf '%s\n' '#!/bin/sh'
  printf '%s\n' 'set -eu'
  printf 'export FAME_CODEX_LAUNCHER_EXECUTABLE=%s\n' "$WRAPPER_QUOTED"
  printf 'export FAME_CODEX_APPIMAGE=%s\n' "$TARGET_QUOTED"
