...
test("startup failure stays visible on another launch and Retry exits the failed instance", async () => {
  ...
  const sandbox = {
    ...
    launchEnvironment: { FAME_CODEX_HOME: undefined, CODEX_HOME: "original-codex-home" },
    process: { argv: ["launcher", "--hidden"], env: { FAME_CODEX_HOME: "dev-home", CODEX_HOME: "dev-codex-home" } },
    dialog: {
