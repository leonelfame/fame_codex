...
test("health proves that Codex received a successful augmented model catalog", async () => {
  const previousHome = process.env.FAME_CODEX_HOME;
  const isolatedHome = mkdtempSync(join(tmpdir(), "fame-codex-model-health-"));
  process.env.FAME_CODEX_HOME = isolatedHome;
  const config = { ...defaultConfig("browser-only"), port: 0 };
  const server = startServer(config, {
  ...
  try {
    ...
    await server.stop(true);
    if (previousHome === undefined) delete process.env.FAME_CODEX_HOME;
    else process.env.FAME_CODEX_HOME = previousHome;
