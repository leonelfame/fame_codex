...
test("DEV setup child environment removes launcher-rebound production aliases", async () => {
  ...
  assert.deepEqual(fixture.host.devSetupEnvironment({
    ...
    FAME_CODEX_DEV_HOME: "/stale-dev",
    FAME_CODEX_LAUNCHER_DATA_DIR: "/dev/launcher",
  }), {
    KEEP_ME: "yes",
    FAME_CODEX_DEV_HOME: path.resolve("/dev"),
  });
...
test("connector verification uses the current identity and rejects a legacy local runtime", () => {
  ...
  const legacyFull = hostFor({ mode: "full", appName: "Codex Native2" });
  ...
  const browserOnly = hostFor({ mode: "browser-only", appName: "Codex Native2" });
