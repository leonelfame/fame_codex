...
test("launcher runtime validation accepts native Windows paths and a named pipe", () => {
  ...
  const config = {
    ...
    autoApproveToolCalls: false,
    controlToken: "runtime-supervisor-control-token-0123456789abcdef",
    runtimeCommand: ["C:\\Users\\Example\\.fame-codex\\runtime\\bun.exe"],
  };
...
test("launcher delegates long-lived tunnel supervision to native runtimes connect", () => {
  const config = launcherConfig("C:\\Users\\Example\\.fame-codex\\runtime\\launcher-browser.json", {
    mode: "full",
    runtimeCommand: [
      "C:\\Users\\Example\\.fame-codex\\versions\\0.2.0-win32-x64\\runtime\\bun.exe",
      "C:\\Users\\Example\\.fame-codex\\versions\\0.2.0-win32-x64\\app\\cli.js",
    ],
    brokerSocketPath: "\\\\.\\pipe\\codex-chatgpt-web-example",
    tunnel: {
      binaryPath: "C:\\Users\\Example\\.fame-codex\\bin\\tunnel-client.exe",
      tunnelId: "tunnel_0123456789abcdef0123456789abcdef",
      runtimeKeyFile: "C:\\Users\\Example\\.fame-codex\\secrets\\tunnel-runtime.key",
      profileDir: "C:\\Users\\Example\\.fame-codex\\tunnel\\profiles",
      profileName: "codex-chatgpt-web",
      alias: "codex-chatgpt-web",
    },
  });
...
test("first launcher tunnel startup creates its missing profile and keeps the verified runtime alive", {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "fame-codex-first-tunnel-"));
  const health = await localHealthServer(() => 200, pathname => pathname.startsWith("/api/logs")
    ? JSON.stringify({ events: [] }) : "ok");
  const profileDir = path.join(root, "profiles");
...
test(`launcher supervisor forwards exact trace cancellation: ${reason ?? "user close"}`, async () => {
  ...
  supervisor.readConfig = () => ({ controlToken: "control", host: "127.0.0.1", port: 17842 });
