...
test("launcher publishes native packages for all supported desktop operating systems", () => {
  assert.equal(manifest.name, "fame-codex-launcher");
  assert.equal(manifest.build.appId, "com.leonelfame.famecodex.launcher");
  assert.equal(manifest.build.productName, "Fame Codex");
...
test("release installers resolve checksummed native launcher assets", () => {
  ...
  assert.match(runtimeBuilder, /FAME_CODEX_NOTICE\.md/);
  assert.doesNotMatch(runtimeBuilder, /join\(root, "LICENSE"\)/);
...
test("release and terminal installer preserve the upstream MIT notice", () => {
  const release = fs.readFileSync(path.join(repositoryRoot, ".github", "workflows", "release.yml"), "utf8");
  const installer = fs.readFileSync(path.join(repositoryRoot, "scripts", "install.sh"), "utf8");
  const notice = fs.readFileSync(
    path.join(repositoryRoot, "LICENSES", "codex-chatgpt-web-MIT.txt"),
    "utf8",
  );
  assert.match(release, /cp LICENSES\/codex-chatgpt-web-MIT\.txt release-assets\/codex-chatgpt-web-MIT\.txt/);
  assert.match(installer, /codex-chatgpt-web-MIT\.txt/);
  assert.match(notice, /Copyright \(c\) 2026 codex-chatgpt-web contributors/);
