...
const roots: string[] = [];
afterEach(() => {
  delete process.env.FAME_CODEX_HOME;
...
test("Windows uses a stable native named pipe for the outer Codex tool broker", () => {
  const first = defaultBrokerEndpoint("C:\\Users\\alice\\.fame-codex", "win32");
  const second = defaultBrokerEndpoint("C:\\Users\\alice\\.fame-codex", "win32");
  expect(first).toBe(second);
  expect(isWindowsPipeEndpoint(first)).toBe(true);
  expect(resolveBrokerEndpoint(first)).toBe(first);
  expect(defaultBrokerEndpoint("/home/alice/.fame-codex", "linux")).toEndWith(join("runtime", "turn-broker.sock"));
});
...
test("setup repairs a legacy automatic connector name that collides with Zero Risk", () => {
  ...
  roots.push(root);
  process.env.FAME_CODEX_HOME = root;
...
test("launcher browser ownership is explicit in provider configuration", () => {
  ...
  config.browserHostDescriptorPath = "/Users/example/.fame-codex/runtime/launcher-browser.json";
