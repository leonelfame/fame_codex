...
function baseConfig(
  ...
  if (config.browserInteractionMode === "manual") {
    ...
    if (config.mode !== "full") {
      throw new Error("Zero Risk requires --full so Fame Codex Zero Risk can signal start, tools, and completion");
    }
...
export async function setup(options: SetupOptions): Promise<SetupResult> {
  ...
  if (beforeService.loaded && !existing) {
    throw new Error("A Fame Codex service is loaded but its configuration is missing; refusing to replace an unverifiable process");
  }
