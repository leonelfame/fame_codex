...
test("production launcher resolves the Fame Codex identity and durable paths", () => {
  const homeDir = path.resolve("/Users/tester");
  ...
  assert.equal(production.displayName, "Fame Codex");
  assert.equal(production.coreHome, path.join(homeDir, ".fame-codex"));
  assert.equal(production.userData, path.join(appData, "Fame Codex"));
  assert.equal(production.browserPartition, "persist:fame-codex-chatgpt");
});
