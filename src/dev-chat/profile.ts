...
export function activateDevProfileEnvironment(paths = resolveDevProfilePaths()): DevProfilePaths {
  ...
  process.env.FAME_CODEX_HOME = paths.home;
...
export function installedLauncherCandidates({
  ...
  const override = environment.FAME_CODEX_LAUNCHER_EXECUTABLE?.trim()
    || environment.CODEX_WEB_GPT_LAUNCHER_EXECUTABLE?.trim();
  ...
  if (platform === "darwin") {
    ...
    candidates.push(posix.join(homeDirectory, ".local", "bin", "fame-codex"));
    for (const entry of (environment.PATH || "").split(":").filter(Boolean)) {
      candidates.push(posix.join(entry, "fame-codex"));
    }
  }
...
export function devLauncherEnvironment(
  ...
  delete childEnvironment.FAME_CODEX_HOME;
  delete childEnvironment.CODEX_HOME;
  delete childEnvironment.FAME_CODEX_LAUNCHER_DATA_DIR;
